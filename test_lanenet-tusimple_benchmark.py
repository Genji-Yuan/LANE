...
def init_args():
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_dir', type=str, default='E:/Dataset/Tusimple/test_set', help='path to TuSimple Benchmark dataset')
    parser.add_argument('--ckpt_path', type=str, default='./checkpoints/ckpt_FCN-Res18-1E1D-b1d0.01-v0.2d1_epoch-590.pth', help='path to parameter file (.pth)')
    parser.add_argument('--arch', type=str, default='fcn', help='network architecture type(default: FCN)')     #fcn, enet,icnet
    parser.add_argument('--dual_decoder', default=False, help='use seperate decoders for two branches')
    parser.add_argument('--ipm', action='store_true', help='whether to perform Inverse Projective Mapping(IPM) before curve fitting')
    parser.add_argument('--show', action='store_true', help='whether to show visualization images when testing')
    parser.add_argument('--save_img',type=str, default=True, help='whether to save visualization images when testing')
    parser.add_argument('--tag', type=str, help='tag to log details of experiments')
